<template>
	<view class="">
		...
		<view class="contentBox m-t3">
			<view class="itemBox flex al-center ju-between" @click="fillIn(item,index)" v-for="(item,index) in formlist"
				:key="item.id" :class="index==formlist.length-1?'dv':''">
				{{item.titel}}
				<view class="flex al-center">
					<input :type="item.style" :placeholder="item.placeholder" v-model="item.value"
						:disabled="item.disabled" class="ipt" />
					<image v-if="index!=4" class="reimg"
		...
		<u-select v-model="Isfloor" title='请选择楼层' mode="mutil-column-auto" :default-value='defaultFloor'
			@confirm="confirmFloor" :list="listFloor"></u-select>
		<!-- 装修 -->
		<u-select v-model="fitmentShow" title='请选择装修' :default-value='defaultFitment' @confirm="confirmFitment"
			:list="fitmentList"></u-select>
		<!-- 电梯 -->
		<u-select v-model="elevatorShow" title='请选择是否有电梯' :default-value='defaultElevator' @confirm="confirmElevator"
			:list="elevatorList"></u-select>
		<!-- 押金 -->
		<u-select v-model="cashShow" title='请选择押金' :default-value='defaultCash' mode="mutil-column"
			@confirm="confirmCash" :list="cashList"></u-select>
		<view class="contentBox m-t3">
			...
			<view @click="cashShow = true" class="itemBox flex al-center dv ju-between">
				...
				<view class="flex al-center">
					...
					<image class="reimg" src="https://oss.kuaitongkeji.com/static/img/app/address/retrue.png" mode="">
					</image>
				</view>
			</view>
		</view>
		...
		<view class="contentBox m-t3 ">
			...
			<view class="flex al-center flex-w">
				<view class=" pos-rel" v-for="(item,index) in image" :key='item.id'>
					...
					<view @click="delImage(index)" class="delBox flex al-center ju-center">
						<image src="https://oss.kuaitongkeji.com/static/img/app/lookroom/del.png" class="delImg"
							mode=""></image>
					</view>
				</view>
				<view @click="selPhoto" class="selphoto flex al-center ju-center">
					<image src="https://oss.kuaitongkeji.com/static/img/app/lookroom/photo.png" class="photoImg"
						mode=""></image>
				</view>
			</view>
			<view class="roomtetx">
			...
			<view class="p-b2 flex">
				...
				<view @click="celCover" class="selphoto flex al-center ju-center">
					<image src="https://oss.kuaitongkeji.com/static/img/app/lookroom/cover.png" class="coverImg"
						mode=""></image>
				</view>
			</view>
		</view>
		...
		<view class="contentBox m-t3">
			...
			<view class=" p-b3">
				<textarea class="textAr" auto-height="true" maxlength="1000" value="" v-model="textvalue"
					placeholder="请填写" />
			</view>
		</view>
		...
		<view class="flex ju-center ">
			<view @click="iSfill" class="submit flex al-center ju-center">
				<image src="https://oss.kuaitongkeji.com/static/img/app/lookroom/submit.png" class="submitImg" mode="">
				</image>
...
<style scoped lang="scss">
	...
	/deep/ .u-select__header[data-v-a577ac80] {
		color: #666666;
	}
